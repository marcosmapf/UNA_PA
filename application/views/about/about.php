<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>Teste</title>
	<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-alpha.6/css/bootstrap.min.css" integrity="sha384-rwoIResjU2yc3z8GV/NPeZWAv56rSmLldC3R/AZzGRnGxQQKnKkoFVhFQhNUwEyJ" crossorigin="anonymous"> <!--Bootstrap 4 Stylesheet Import-->
	<link rel="stylesheet" href="../../../assets/css/about.css"> <!--Local Stylesheet Import-->
	<link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet"> <!--Material Icons Font-->
	<script src="http://code.jquery.com/jquery-3.2.1.min.js"></script>
</head>
<body ng-app="appModule" ng-cloak>

	<div class="container">

		<div class="row descricao-projeto">
			<div class="col-xl-3 col-lg-3 col-md-12 col-sm-12 col-xs-12 col-12">
				<a href="../main/main.php">
					<img src="../../../assets/imagens/logo-media.png">
				</a>
			</div>
			<div class="col-xl-9 col-lg-9 col-md-12 col-sm-12 col-xs-12 col-12">
				<h5>Pet Stop E-Commerce é um projeto desenvolvido pelos seguintes alunos do curso de Sistemas de Informação do Centro Universitário UNA, 6º período. Os membros do grupo desenvolveram o site e todo seu sistema backend do zero e, para tanto, tiveram de aprender diversas técnicas de desenvolvimento e ferramentas que certamente irão contribuir para seu desenvolvimento no futuro. O grupo agradece pela oportunidade oferecida!
				</h5>
			</div>
		</div>

		<main class="card">

			<div id="dev-felippe" class="dev-card">
				<div class="dev-img">
					<img class="foto-desenvolvedor" src="../../../assets/imagens/foto-felippe.jpg">
				</div>
				<div class="dev-info">
					<div class="product-text">
						<h1>Felippe Weber</h1>
						<h4>Desenvolvedor Front-end</h4>
						<p></p>
					</div>
					<div class="social-media d-inline-block">
						<a href="https://www.facebook.com/felippe.galo13"><i class="fa fa-facebook-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://twitter.com/felippetaw"><i class="fa fa-twitter-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://br.linkedin.com/in/felippe-weber-4261a1b6"><i class="fa fa-linkedin-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.instagram.com/felippetaw/"><i class="fa fa-instagram fa-2x" aria-hidden="true"></i></a>							
					</div>
				</div>
			</div>

			<div id="dev-marcos" class="dev-card">
				<div class="dev-img">
					<img class="foto-desenvolvedor" src="../../../assets/imagens/foto-marcos.jpg">
				</div>
				<div class="dev-info">
					<div class="product-text">
						<h1>Marcos de Andrade</h1>
						<h4>Desenvolvedor Full-stack</h4>
						<p></p>
					</div>
					<div class="social-media d-inline-block">
						<a href="https://www.github.com/marcosmapf"><i class="fa fa-github-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.facebook.com/mapfmarcos"><i class="fa fa-facebook-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://twitter.com/marcosscamor"><i class="fa fa-twitter-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://br.linkedin.com/in/marcos-de-andrade-pinto-filho"><i class="fa fa-linkedin-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.instagram.com/marcosmapf/"><i class="fa fa-instagram fa-2x" aria-hidden="true"></i></a>							
					</div>
				</div>
			</div>

			<div id="dev-marina" class="dev-card">
				<div class="dev-img">
					<img class="foto-desenvolvedor" src="../../../assets/imagens/foto-marina.jpg">
				</div>
				<div class="dev-info">
					<div class="product-text">
						<h1>Marina Emelly</h1>
						<h4>Desenvolvedora Full-stack</h4>
						<p</p>
					</div>
					<div class="social-media d-inline-block">
						<a href="https://github.com/marinaemelly"><i class="fa fa-github-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.instagram.com/marinaemelly/"><i class="fa fa-linkedin-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.linkedin.com/in/marina-jeronimo-0b83aaa6/"><i class="fa fa-instagram fa-2x" aria-hidden="true"></i></a>			
					</div>
				</div>
			</div>

			<div id="dev-messias" class="dev-card">
				<div class="dev-img">
					<img class="foto-desenvolvedor" src="../../../assets/imagens/foto-messias.jpg">
				</div>
				<div class="dev-info">
					<div class="product-text">
						<h1>Messias Martins</h1>
						<h4>Desenvolvedor Back-end</h4>
						<p></p>
					</div>
					<div class="social-media d-inline-block">
						<a href="https://www.facebook.com/messias.martins.5"><i class="fa fa-facebook-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.linkedin.com/in/messiasmartins/"><i class="fa fa-linkedin-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.instagram.com/messiasmartinss/"><i class="fa fa-instagram fa-2x" aria-hidden="true"></i></a>						
					</div>
				</div>
			</div>

			<div id="dev-valdirene" class="dev-card">
				<div class="dev-img">
					<img class="foto-desenvolvedor" src="../../../assets/imagens/foto-valdirene.jpg">
				</div>
				<div class="dev-info">
					<div class="product-text">
						<h1>Valdirene Aparecida</h1>
						<h4>Desenvolvedor Back-end</h4>
						<p></p>
					</div>
					<div class="social-media d-inline-block">
						<a href="https://www.facebook.com/valdireneaparecida.silva"><i class="fa fa-facebook-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://twitter.com/val_apasilva"><i class="fa fa-twitter-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.linkedin.com/in/valdirene-aparecida-72756065/"><i class="fa fa-linkedin-square fa-2x" aria-hidden="true"></i></a>
						<a href="https://www.instagram.com/val_apasilva/"><i class="fa fa-instagram fa-2x" aria-hidden="true"></i></a>							
					</div>
				</div>
			</div>
		</main>
	</div>
</body>
<script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script> <!--Jquery Import-->
<script type="text/javascript" src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-alpha.6/js/bootstrap.min.js" integrity="sha384-vBWWzlZJ8ea9aCX4pEW3rVHjgjt7zpkNpZk+02D9phzyeVkE+jo0ieGizqPLForn" crossorigin="anonymous"></script> <!--Bootstrap 4 Import-->
<script src="../../../assets/node_modules/angular/angular.js"></script> <!--AngularJS Import-->
<script src="../../../assets/node_modules/angular-animate/angular-animate.js"></script> <!--Angular Material Dependency (Animations) Import-->
<script src="../../../assets/node_modules/angular-aria/angular-aria.js"></script> <!--Angular Material Dependency (Accessibility) Import-->
<script src="../../../assets/node_modules/angular-messages/angular-messages.js"></script> <!--Angular Material Dependency (Angular Messages) Import--> 
<script src="../../../assets/node_modules/angular-material/angular-material.js"></script> <!--Angular Material Import-->
<script src="../../../assets/node_modules/angular-ui-mask/dist/mask.js"></script>
<script type="text/javascript" src="https://use.fontawesome.com/574c45c449.js"></script> <!--Font Awesome Import-->
<script type="text/javascript" src="../../../assets/js/cadastro.js"></script> <!--Local Javascript Import-->
</html>

<!-- 
Tutoriais Angular Messages 	
https://www.yearofmoo.com/2014/05/how-to-use-ngmessages-in-angularjs.html 
https://docs.angularjs.org/api/ngMessages/directive/ngMessages
-->